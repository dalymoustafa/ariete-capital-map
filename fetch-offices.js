...
async function main() {
  ...
  const artonRecord = records.find(r =>
    r.fields["Name"] && r.fields["Name"].toLowerCase().includes("ariete capital")
  );

  if (!artonRecord) {
    console.error("Could not find Ariete Capital!");
    process.exit(1);
  }

  console.log("Found Ariete Capital! Fields:", JSON.stringify(Object.keys(artonRecord.fields)));
